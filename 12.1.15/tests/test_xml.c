...
main (void)
{
  ...
  jb_xml_node_set_float_with_default (node, XML_X,
                                      (JBDOUBLE) 1., (JBDOUBLE) 0.);
  jb_xml_node_set_int_with_default (node, XML_I, 1, 0);
  jb_xml_node_set_time_with_default (node, XML_T, (JBDOUBLE) 1., (JBDOUBLE) 0.);
  ...
  x = jb_xml_node_get_float_with_default (node, XML_X, &e1, (JBDOUBLE) 0.);
  SNPRINTFL (str, JB_BUFFER_SIZE, FWL, x);
  printf ("x %s 1\n", str);
  i = jb_xml_node_get_int_with_default (node, XML_I, &e2, 0);
  printf ("i %d 1\n", i);
  x = jb_xml_node_get_time_with_default (node, XML_T, &e3, (JBDOUBLE) 0.);
  SNPRINTFL (str, JB_BUFFER_SIZE, FWL, x);
  printf ("t %s 1\n", str);
  if (!e1 || !e2 || !e3)
    {
